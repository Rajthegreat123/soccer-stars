...
import { GameState, Player, Disc, Ball } from "../schema/GameState";
...
export class SoccerRoom extends Room<GameState> {
  ...
  onLeave(client: Client, consented: boolean) {
    ...
    this.state.discs.forEach((disc: Disc, key: string) => {
  ...
  private createPlayerDiscs(playerId: string, team: number) {
    const formations = [
  ...
  private handleMove(client: Client, message: any) {
    ...
    if (!player || this.state.gamePhase !== 1) return;
    
    // Check if it's player's turn
    if (player.team !== this.state.currentTurn) return;
    
    // Check if still moving
    if (this.state.isMoving) return;
    
    const disc = this.state.discs.get(message.discId);
    if (!disc || disc.team !== player.team) return;
  ...
  private handleShoot(client: Client, message: any) {
    const player = this.state.players.get(client.sessionId);
    if (!player || this.state.gamePhase !== 1) return;
  ...
  private update(deltaTime: number) {
    if (this.state.gamePhase !== 1) return;
    ...
    // Update disc physics
    this.state.discs.forEach((disc: Disc) => {
      // Apply friction
      disc.vx *= friction;
      disc.vy *= friction;
      
      // Stop if velocity too low
      if (Math.abs(disc.vx) < minVelocity) disc.vx = 0;
      if (Math.abs(disc.vy) < minVelocity) disc.vy = 0;
      ...
      // Boundary collision
      if (disc.x < 20) { disc.x = 20; disc.vx = -disc.vx * 0.7; }
      if (disc.x > this.state.fieldWidth - 20) { disc.x = this.state.fieldWidth - 20; disc.vx = -disc.vx * 0.7; }
      if (disc.y < 20) { disc.y = 20; disc.vy = -disc.vy * 0.7; }
      if (disc.y > this.state.fieldHeight - 20) { disc.y = this.state.fieldHeight - 20; disc.vy = -disc.vy * 0.7; }
  ...
  private updateBallPhysics(friction: number, minVelocity: number) {
    // Apply friction
    this.state.ball.vx *= friction;
    this.state.ball.vy *= friction;
    
    // Stop if velocity too low
    if (Math.abs(this.state.ball.vx) < minVelocity) this.state.ball.vx = 0;
    if (Math.abs(this.state.ball.vy) < minVelocity) this.state.ball.vy = 0;
    ...
    // Boundary collision
    if (this.state.ball.x < 15) { this.state.ball.x = 15; this.state.ball.vx = -this.state.ball.vx * 0.8; }
    if (this.state.ball.x > this.state.fieldWidth - 15) { this.state.ball.x = this.state.fieldWidth - 15; this.state.ball.vx = -this.state.ball.vx * 0.8; }
    if (this.state.ball.y < 15) { this.state.ball.y = 15; this.state.ball.vy = -this.state.ball.vy * 0.8; }
    if (this.state.ball.y > this.state.fieldHeight - 15) { this.state.ball.y = this.state.fieldHeight - 15; this.state.ball.vy = -this.state.ball.vy * 0.8; }
  }
